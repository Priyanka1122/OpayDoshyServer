...
async function addUser(req, res, next) {
	try {
		...
		if (mobile === undefined || mobile === null || mobile === "")
			return res.json({ status: false, msg: "Please provide the mobile number." });
...
async function VerifyOtp(req, res, next) {
	...
	try {
		...
		if (otp === undefined || otp === null || otp === "")
			return res.json({ status: false, msg: "Please provide the otp number." });
		customer = Customer;
...
async function updateDetails(req, res, next) {
	try {
		...
		if (dob === undefined || dob === null || dob === "")
			return res.json({ status: false, msg: "Please provide the dob." });
...
async function pinSetting(req, res, next) {
	try {
		...
		if (pin === undefined || pin === null || pin === "")
			return res.json({ status: false, msg: "Please provide the PIN Number." });
		customer = Customer;
		customer.updateOne(
			{ auth_key: auth_key },
			{
		...
		var serverKey =
			"AAAA0YNs_0U:APA91bGv9e0tfAIm9PhMG-bfjzOvFivXFEemBQb9Tz7SoKqyctWN-AGp0mGnDoHGPQg2vmx2ubUCXSAaELzQ_7bVxGoLpLe06haVq9avG2c0oCYUNBSCXWX2hb58pvslMWC_ihBve7bW";
...
async function sendmail(req, res, next) {
	try {
		...
		return res.json({
			status: true,
			msg: "mail verification link has been send to your E-mail.",
			type: "confirmation"
		});
	} catch (err) {
		return res.status(401).send({ status: false, msg: "Something Went Wrong.Please Try Again!" });
	...
	try {
		...
		if (user.emailverified == false) {
			fs.readFile("./logic/verfication.html", function(error, data) {
				customer = Customer;
				customer.updateOne({ verificationLink: verificationLink }, { emailverified: true }, function(
					err,
					affected,
					resp
				) {
...
async function changesPin(req, res, next) {
	try {
		...
		if (current_pin === undefined || current_pin === null || current_pin === "")
			return res.json({ status: false, msg: "Please provide the current pin." });

		if (user.pin.toString() !== current_pin)
			return res.json({ status: false, msg: `Current PIN entered is incorrect` });

		if (new_pin === undefined || new_pin === null || new_pin === "")
			return res.json({ status: false, msg: "Please provide the new pin." });
...
async function touchStatus(req, res, next) {
	try {
		...
		if (touch_status === undefined || touch_status === null || touch_status === "")
			return res.json({ status: false, msg: "Please provide the touch status." });
...
async function notification_app_status(req, res, next) {
	try {
		...
		if (notification_app_status === undefined || notification_app_status === null || notification_app_status === "")
			return res.json({ status: false, msg: "Please provide the notification app status." });

		customer = Customer;
		customer.updateMany({ auth_key: auth_key }, { notification_app_status: notification_app_status }, function(
			err,
			affected,
			resp
		) {});
...
async function notification_sms_status(req, res, next) {
	try {
		...
		if (notification_sms_status === undefined || notification_sms_status === null || notification_sms_status === "")
			return res.json({ status: false, msg: "Please provide the notification sms status." });

		customer = Customer;
		customer.updateMany({ auth_key: auth_key }, { notification_sms_status: notification_sms_status }, function(
			err,
			affected,
			resp
		) {});
...
async function logindata(req, res, next) {
	try {
		...
		if (pin === undefined || pin === null || pin === "")
			return res.json({ status: false, msg: "Please provide the  pin." });
...
async function notificationlist(req, res, next) {
	try {
		...
		let notificationdata = await Notificationlist.find({ auth_key: auth_key }).sort({ createdAt: -1 });
...
async function touch_status(req, res, next) {
	try {
		...
		if (touch_status === undefined || touch_status === null || touch_status === "")
			return res.json({ status: false, msg: "Please provide the touch app status." });
