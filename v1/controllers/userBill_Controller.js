...
async function  newbilldata(req,res,next) {
    try{
        ...
        const {Bill_OID,User_Mobile,Biller_OID,Biller_Name,Biller_Bill_ID,Bill_Amount,Bill_Due_Date,Direct_Debit_Date,Direct_Debit_Amount,Link_Attachment,Bpay_Biller_ID,Bpay_Biller_CRN} =req.body;
   ...
   let user = await Customer.findOne({ auth_key: auth_key});//Finding the specific User
    ...
    if (Bill_OID === undefined || Bill_OID === null || Bill_OID === "") return res.json({ status: false, msg: 'Please provide the bill oid.' });

    if (User_Mobile === undefined || User_Mobile === null || User_Mobile === "") return res.json({ status: false, msg: 'Please provide the user mobile.' });

    //if (Biller_OID === undefined || Biller_OID === null || Biller_OID === "") return res.json({ status: false, msg: 'Please provide the user biller oid.' });

    if (Biller_Name === undefined || Biller_Name=== null || Biller_Name === "") return res.json({ status: false, msg: 'Please provide the user biller name.' });

    if (Biller_Bill_ID === undefined || Biller_Bill_ID=== null || Biller_Bill_ID === "") return res.json({ status: false, msg: 'Please provide the user biller bill id.' });

    if (Bill_Amount === undefined || Bill_Amount=== null || Bill_Amount === "") return res.json({ status: false, msg: 'Please provide the user bill amount.' });
 
    if (Bill_Due_Date === undefined || Bill_Due_Date=== null || Bill_Due_Date === "") return res.json({ status: false, msg: 'Please provide the user bill due date.' });

    if (Direct_Debit_Date === undefined || Direct_Debit_Date=== null || Direct_Debit_Date=== "") return res.json({ status: false, msg: 'Please provide the direct debit date.' });
...
async function billerdata(req,res,next) {
    try{
        ...
        const {Biller_OID,Biller_Name,Biller_Entity_Legal_Name,Biller_ABN_ACN,Biller_Business_Address,Biller_Postal_Address,Biller_Contact_Person,Biller_Contact_Number,Biller_Contact_Email,Biller_Logo}=req.body;
